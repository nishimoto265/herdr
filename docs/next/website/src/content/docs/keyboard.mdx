...
When the Review Agent has pending rule proposals, Herdr opens a collapsible panel on the right. While it is collapsed, a `[‹ Rules N]` handle sits at the right edge of the terminal area, where `N` is the pending proposal count; click it to open the panel manually. With no pending proposals the handle dims to `[‹ Rules]`. The panel folds automatically after the last pending proposal is approved or rejected, and you can close it manually at any time with the `[›]` control in its top-right corner.
