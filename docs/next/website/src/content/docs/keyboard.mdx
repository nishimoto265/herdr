...
Every pane slot has a resident backside from the moment the pane is created. The front and back are independent terminal processes with separate scrollback and sessions. `prefix+f` switches which side is displayed and receives keyboard, mouse, scroll, and copy-mode input; the hidden side keeps running.
