...
Every pane slot has a resident backside from the moment the pane is created. The front and back are independent terminal processes with separate scrollback and sessions. `prefix+f` switches which side is displayed and receives keyboard, mouse, scroll, and copy-mode input; the hidden side keeps running. Herdr marks a nonfocused displayed backside with a mauve `BACK` border label. A focused or selected backside keeps its normal accent border while retaining the `BACK` label. When a pane has no border, Herdr shows a small `BACK` badge at the top-right instead.
