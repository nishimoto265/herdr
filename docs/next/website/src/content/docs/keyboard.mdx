...
When the Review Agent has pending rule proposals, Herdr opens a collapsible panel on the right. Click the right-side rail to open it manually. The panel folds automatically after the last pending proposal is approved or rejected, and you can close it manually at any time.

With the review panel focused, use `up`/`k` and `down`/`j` to select a proposal, `pageup` and `pagedown` to scroll, `a` to approve, `r` to reject, and `esc` or `q` to close the panel. The supported CLI intentionally has no approve or reject command.
