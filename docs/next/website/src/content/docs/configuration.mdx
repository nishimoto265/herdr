...
The **shitsuji** (Japanese for butler) agent is opt-in. It watches completed Claude and Codex turns, asks a dedicated backend agent to review them, and presents proposed rules in Herdr's right-side rule proposal panel.
...
- `enabled` enables the feature. It defaults to `false`.
- `backend_profile_id` identifies the backend profile associated with the shitsuji session. It must not be empty.
- `backend_argv` is the executable and arguments used to replace the pane's resident backside with the shitsuji backend. Each array item is passed directly as an argument; Herdr does not parse this value through a shell.
- `readiness_attempts` sets the maximum number of transcript-readiness checks. Values from `2` through `100` are accepted.
- `readiness_interval_ms` sets the delay between readiness checks. Values from `1` through `5000` milliseconds are accepted.

Hot-reloads through `herdr server reload-config`, so `[shitsuji_agent]` is not a startup-only section. A reload that turns the runtime off — through `enabled = false` or through a value Herdr rejects — stops the shitsuji backends and discards completed turns that have not reached a backend yet. Proposed and approved rules are kept.

Herdr does not install or depend on Claude or Codex hooks. It uses the same screen-based agent state detection as the rest of Herdr, binds the transcript when a detected agent starts working, and checks the provider completion boundary when that agent returns to idle.

Claude transcripts are resolved below `$CLAUDE_CONFIG_DIR/projects`, or `~/.claude/projects` when that variable is unset. Codex transcripts are resolved below `$CODEX_HOME/sessions`, or `~/.codex/sessions` when that variable is unset. Herdr matches the current working directory and available session identity, and does not choose a transcript when multiple candidates remain ambiguous.

Before notifying the backend, Herdr waits for the provider-specific completion boundary and two stable observations of the JSONL file, up to the configured attempt limit. It then passes the canonical absolute JSONL path and the completed byte range to the backend. The path must remain a regular, non-symlinked file inside the provider's data root.

:::caution
`backend_argv` is a trusted local executable that runs with your user account's permissions. It can reach the same local sockets and session files as other processes owned by your account, so the rule proposal panel is a workflow boundary, not a security sandbox against a malicious backend. Configure only an executable and profile you trust. Conversation transcripts remain untrusted input and may contain prompt-injection attempts; the backend must treat transcript content as data, never as instructions to execute.
:::
