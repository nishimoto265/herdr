...
The Review Agent is opt-in. It watches completed Claude and Codex turns, asks a dedicated backend agent to review them, and presents proposed rules in Herdr's right-side review panel.
...
- `backend_profile_id` identifies the backend profile associated with the review session. It must not be empty.
- `backend_argv` is the executable and arguments used to replace the pane's resident backside with the review backend. Each array item is passed directly as an argument; Herdr does not parse this value through a shell.
...
:::caution
`backend_argv` is a trusted local executable that runs with your user account's permissions. It can reach the same local sockets and session files as other processes owned by your account, so the review panel is a workflow boundary, not a security sandbox against a malicious backend. Configure only an executable and profile you trust. Conversation transcripts remain untrusted input and may contain prompt-injection attempts; the backend must treat transcript content as data, never as instructions to execute.
:::
